...
Settled 2026-08-09 by `androidpublisher` `tracks.get` (read-only scratch edit,
deleted uncommitted, nothing staged): **production serves versionCode 6**,
release name 1.0.0, status `completed`. The internal track serves versionCode 5;
alpha is an empty draft; beta is empty. So versionCode 5 and 6 WERE promoted
after the 2026-07-31 vc4 release, and the "Updated on Aug 3, 2026" stamp on the
public listing is the vc6 promotion.

The general caveat still holds: a finished EAS build is not a Play release. The
EAS build record alone never establishes what a track serves -- that needs
`androidpublisher` `tracks.get` or the Console.
