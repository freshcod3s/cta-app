...
**Amended 2026-08-10.** Row 1 audited the title as "Congress Stock Trade
Alerts" (27/30) on the strength of its being locked copy. That rename is
REJECTED — the name stays "Congress Trade Alerts" (21/30), matching
congresstradealerts.com and the other properties named for it. Row 1 now
audits the reverted title. The audit's substance is unchanged: the alerting
claim row 1 verified holds for either wording, and no other row cites the
title. Note the rejected name WAS committed to Play on 2026-08-09 and is
sitting approved-but-unpublished in the pending change set; this file governs
staged intent, not that Console state.

**Corrected later the same day, 2026-08-10.** The sentence above went stale
within hours: the pending set was PUBLISHED on 2026-08-10, so the rejected
27-character name is what play.google.com serves today. Row 1 audits the
LOCKED name (Product Invariant 10), which is the value this repo carries and
the value the corrective Console edit will restore -- not the value currently
serving. A corrective listing edit was saved in Console on 2026-08-10 and is
sitting under "Changes not yet submitted for review"; sending it is Joe's
Console step.

| # | Claim (verbatim) | Shipped proof | Verdict |
|---|---|---|---|
| 1 | Title: "Congress Trade Alerts" (21/30) | The LOCKED name (Product Invariant 10). ~~matches the title serving on the live listing today~~ — **corrected 2026-08-10: it does NOT match what is serving.** The 2026-08-10 publish put the rejected 27-char name live; the corrective Console edit is saved but unsent. Alerting is real: subscribe surface + push token pipeline (rows 7–9). | PASS |
