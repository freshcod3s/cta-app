...
| Currently on Play production | versionCode 4 (released 2026-07-31) | `docs/android-launch-checklist.md:189`, `CLAUDE.md:409-412` |
| Expected vc7 versionCode | **7** -- assumption, must be confirmed in step 3 | derived from "last issued 6" |
...
```bash
cd /c/Users/jcros/Projects/cta-app && git log --oneline -4 && git status --porcelain
```
...
```bash
cd /c/Users/jcros/Projects/cta-app && grep -rniE 'real-?[ -]?time|instant|immediately|as it happens|as they file|the moment|within minutes|as soon as|up to the minute|live feed' app/ features/ lib/ components/ hooks/
```

```bash
cd /c/Users/jcros/Projects/cta-app && grep -rniE '\bpro\b|subscriber|stripe|billing|upgrade|checkout' app/ features/ lib/ components/ hooks/
```
...
```bash
cd /c/Users/jcros/Projects/cta-app && npx eas build:version:get -p android
```
...
```bash
cd /c/Users/jcros/Projects/cta-app && CONFIRM_SPEND=1 npx eas build -p android --profile production
```
...
|---|---|---|
| App name | Replace with `Congress Stock Trade Alerts` | 27/30 |
| Short description | Replace | 77/80 |
| Full description | Replace entire contents with the FULL_DESCRIPTION block, lines 19-51 | **2516/4000** |
...
```bash
cd /c/Users/jcros/Projects/cta-app && python scripts/aso/composite_screenshots.py
```
